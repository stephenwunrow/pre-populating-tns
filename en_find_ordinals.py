...
with tqdm(total=len(chunks), desc="Processing Chunks") as pbar:
    for chunk in chunks:
        ...
        matches = re.findall(r'x-morph="[^"]*?Ao[^"]*?".+?x-content="([^"]+?)".+\\w .+?\|', chunk, re.DOTALL)
        if matches:  # Only proceed if there are matches
            for match in matches:
                lexeme = match  # lexeme is the matched group from x-content
                ...
                if gloss_matches:
                    ...
                    verse_data.append([f'{book_name} {chapter}:{verse}', combined_gloss, lexeme])
...
if verse_data:
    ...
    with open(f'{directory_path}/en_new_ordinals.tsv', 'w', encoding='utf-8') as f:
...
if modified_verse_data:
    with open(f'{directory_path}/transformed_ordinals.tsv', 'w', encoding='utf-8') as outfile:
    ...
    print(f"Transformed data has been written to transformed_ordinals.tsv")
